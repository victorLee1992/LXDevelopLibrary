...
static NSTimeInterval _timeout = 15.0f;                     // 请求超时时间，默认60s
...
+ (NJURLSessionTask *)requestWithUrl:(NSString *)url
    ...
    switch (httpMethod) {
        case NJHttpMedthTypeGET:
            [self getWithManmger:manager absolute:nil session:session Url:absolute
                    refreshCache:refreshCache params:params progress:progress success:success fail:fail];
            break;
        case NJHttpMedthTypePOST:
            [self postWithManmger:manager absolute:nil session:session Url:absolute
                     refreshCache:refreshCache params:params progress:progress success:success fail:fail];
            break;
        case NJHttpMedthTypeUploadImage:
            [self uploadImageWithManmger:manager absolute:nil session:session Url:absolute params:params iamge:image
                                filename:filename name:name mimeType:mimeType progress:progress success:success fail:fail];
            break;
        case NJHttpMedthTypeUploadFile:
            [self uploadFileWithManmger:manager session:session Url:absolute uploadingFile:uploadingFile progress:progress success:success fail:fail];
            break;
        case NJHttpMedthTypeDownload:
            [self downLoadWithManmger:manager session:session Url:absolute saveToPath:saveToPath progress:progress success:success fail:fail];
            break;
        default:
            break;
    }
...
+ (void)postWithManmger:(AFHTTPSessionManager *)manager
    ...
    session = [manager POST:url parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        [[self allTasks] removeObject:task];
        // 缓存响应
        id response = refreshCache ?[NJAFNetworking cahceResponseWithURL:url parameters:params]:nil;
        // 失败回调
        [self failResponse:response callback:fail error:error];
    }];
}
